...
# ---------- Age Estimation (OpenCV DNN AgeNet) ----------
...
# (Optional on Jetson: set preferable backend/target to CUDA if you have it configured)
        ...
        for face_idx, face in enumerate(results.multi_face_landmarks):
            ...
            if x_max > x_min and y_max > y_min:
                ...
                if face_roi.size > 0:
                    ...
                    i = int(age_preds[0].argmax())
                    age_label = AGE_BUCKETS[i]
                    age_conf = float(age_preds[0][i])

            # ====== Label text above the box (Face + Age) ======
            if age_label:
                label = f"Face {face_idx} {age_label} ({age_conf*100:.1f}%)"
            else:
            ...
            # ====== Iris / eye direction ======
            left_iris = np.array([(face.landmark[i].x * w, face.landmark[i].y * h)
                                  for i in range(468, 473)])
            right_iris = np.array([(face.landmark[i].x * w, face.landmark[i].y * h)
            ...
            cv2.arrowedLine(frame, tuple(left_center.astype(int)), gaze_end,
            ...
            right_eye_pts = np.array(
                [[face.landmark[i].x * w, face.landmark[i].y * h] for i in RIGHT_EYE_IDXS],dtype=np.float32
            )
            ...
            cv2.putText(frame, blink_text, (x_min, text_y + th + 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

    cv2.imshow("Multi-face Gaze + Bounding Boxes + Blink Rate + Age", frame)
