...
# Optional CUDA on Jetson if your OpenCV is built with it:
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_idx, face in enumerate(results.multi_face_landmarks):
            ...
            # ====== Age & Gender estimation on face ROI ======
            age_label = ""
            age_conf = 0.0
            gender_label = ""
            gender_conf = 0.0
            ...
            if gender_label and age_label:
                label = f"Face {face_idx} {gender_label} {age_label} ({age_conf*100:.1f}%)"
            elif age_label:
