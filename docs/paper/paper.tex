...
\begin{table}[htbp]
\centering
\caption{All ten architectures on the held-out driver split. Params is the trained classifier only, it does not include the pretrained cascade stages before it. The three classical ML models do not have a comparable parameter count, so they are marked N/A.}
\label{tab:comparison}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Architecture} & \textbf{Input} & \textbf{macro-F1} & \textbf{Params} \\
\midrule
SVM (RBF) & features & 0.531 & N/A \\
Random Forest & features & 0.599 & N/A \\
Gradient Boosted Trees & features & 0.620 & N/A \\
MLP (per-frame) & features & 0.599 & 3{,}101 \\
TCN & features, seq. & 0.720 & 139{,}549 \\
R3D-18 (3D-CNN) & raw pixels & 0.712 & 33{,}177{,}437 \\
ResNet18+GRU (fine-tuned) & raw pixels & 0.742 & 11{,}314{,}205 \\
GRU (single-branch) & features, seq. & 0.790 & 40{,}349 \\
Gated-Fatigue GRU & features, seq. & 0.798 & 44{,}260 \\
\textbf{Ensemble (both GRUs)} & features, seq. & \textbf{0.810} & \textbf{84{,}609} \\
\bottomrule
\end{tabular}
\end{table}
...
% TODO(author): this section is currently just parameter counts and other
% papers' published numbers. Fill in the placeholder table below once we
% actually benchmark on the Jetson.
...
We have not measured our pipeline on the Jetson yet at the time of writing this paper. Table~\ref{tab:embedded_plan} lists what we plan to measure once the deployment is set up.

\begin{table}[htbp]
\centering
\caption{Planned Jetson measurements (not done yet).}
\label{tab:embedded_plan}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Measurement} & \textbf{Status} \\
\midrule
Per-stage latency (①--⑤, TensorRT FP16) & TODO \\
End-to-end pipeline latency / fps & TODO \\
Power draw at 7\,W / 15\,W / 25\,W / MAXN & TODO \\
Peak memory use (8\,GB shared pool) & TODO \\
\bottomrule
\end{tabular}
\end{table}
